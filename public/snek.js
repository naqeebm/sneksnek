...
const sprites = {
	...
	violet: document.createElement('img'),
	bg: document.createElement('img'),
	bgUnderneath: document.createElement('img'),
	food: document.createElement('img')
}

sprites.red.src = "./sprites/red.png"
sprites.green.src = "./sprites/green.png"
sprites.blue.src = "./sprites/blue.png"
sprites.violet.src = "./sprites/violet.png"
sprites.bg.src = "./sprites/bg.png"
sprites.bgUnderneath.src = "./sprites/bgUnderneath.png"
sprites.food.src = "./sprites/food.png"
...
socket.on('data', data => {
	...
	globalOffset = [-1 * (meta.tiles * meta.tileSize / 2) / 3, meta.tiles * meta.tileSize / 2];
	...
	// 	const max = meta.tileSize;
	// 	const min = meta.tileSize / 2;
	// 	const length = snake.blocks.length;
	// 	const incr = (max - min) / length;
	// 	for (let i = length -2; i >= 0; i--) {
	// 		snake.blocks[i].size = Math.max(max - (incr*(length-i)),min);
	// 	}
	// })

	let snakeBlocks = []
	for( let i=0; i<snakes.length; i++){
		for (let j=0; j<snakes[i].blocks.length; j++){
			snakeBlocks.push(snakes[i].blocks[j]);
			if (j>0){
				// let newBlock = snakes[i].blocks[j];
				// newBlock.x = (snakes[i].blocks[j-1].x + snakes[i].blocks[j].x)/2;
				// newBlock.y = (snakes[i].blocks[j-1].y + snakes[i].blocks[j].y)/2;
				// newBlock.size = (snakes[i].blocks[j-1].size + snakes[i].blocks[j].size)/2;
	...
	objectsList = [food, snakeBlocks].flat().sort((obj1, obj2) => {
		if (obj1.x > obj2.x) {
			return -1;
		...
		} else if (obj1.x < obj1.x) {
			return 1;
		}
	});

	objectsList.forEach(gameObj => {
		if (gameObj.type == "snakeBlock") {
			const isoLayer = 1;
			const size = gameObj.size/meta.tileSize;
			const qSize = meta.tileSize / 4;
			let x = (gameObj.x + isoLayer + (1 - size) / 2) * meta.tileSize;
			let y = (gameObj.y - isoLayer + (1 - size) / 2) * meta.tileSize;
			const coords = rotate(0, 0, x, y, 45);
			x = coords[0] + globalOffset[0];
			y = coords[1] / 2 + globalOffset[1];

			ctx.drawImage(sprites.red, x, y, meta.tileSize * size + qSize, meta.tileSize * size + qSize);
		}
		if(gameObj.type == "food"){
			drawFood(ctx,gameObj,meta.tileSize)
		}
	});
	...
	snakes.forEach(snake => {
		...
		if (snake.message !== null) {
			...
			const isoLayer = 1;
			const qSize = meta.tileSize / 4;
			let x = head.x + isoLayer * meta.tileSize;
			let y = head.y - isoLayer * meta.tileSize;
			const coords = rotate(0, 0, x, y, 45);
			x = coords[0] + globalOffset[0];
			y = coords[1] / 2 + globalOffset[1];
...
function getIso(x, y, tileSize = 8) {
	let newx = tileSize * x;
	let newy = tileSize * y
	return [newx, newy];
}
...
// function drawIsoGrid(ctx, tiles, tileSize = 8, offset = 1, col = bgCol) {
// 	const hSize = 0.5;
// 	for (let loopx = 0; loopx < tiles; loopx++) {
// 		for (let loopy = 0; loopy < tiles; loopy++) {
// 			let x = loopx * tileSize;
// 			let y = loopy * tileSize;
// 			const coords = rotate(0, 0, x, y, 45);
// 			x = coords[0];
// 			y = coords[1] / 2;
// 			ctx.fillStyle = "white"
...
function drawSpriteIsoGrid(ctx, tiles, tileSize, offset, col) {
	const qSize = tileSize / 4;
	const hSize = tileSize / 2;
	const fSize = tileSize - offset;
	for (let loopx = tiles-1; loopx >= 0; loopx--) {
		for (let loopy = 0; loopy < tiles; loopy++) {
			let x = loopx * tileSize;
			let y = loopy * tileSize;
			const coords = rotate(0, 0, x, y, 45);
			x = coords[0] + globalOffset[0];
			y = coords[1] / 2 + globalOffset[1];
			ctx.drawImage(sprites.bg, x, y, tileSize + hSize, tileSize + hSize);
			ctx.font = "32pt monospace";
			ctx.fillStyle = "black";
			// ctx.fillText(`${Math.round(loopx)},${Math.round(loopy)}`, x + qSize, y + qSize + qSize / 2);
		}
	}
}
...
function drawGrid(ctx, tiles, tileSize = 8, offset = 1, col = bgCol) {
	drawSpriteIsoGrid(ctx, tiles, tileSize, offset);
	// drawIsoGrid(ctx, tiles, tileSize, offset, col = bgCol);
	// for (let x = 0; x < tiles; x++) {
	//   for (let y = 0; y < tiles; y++) {
	//     ctx.fillStyle = col;
	//     ctx.beginPath();
	//     ctx.moveTo((x * tileSize), (y * tileSize));
	//     ctx.lineTo((x * tileSize)+ (tileSize - offset), y * tileSize);
	//     ctx.lineTo((x * tileSize)+ (tileSize - offset), (y * tileSize)+tileSize - offset);
	//     ctx.lineTo((x * tileSize), (y * tileSize) + tileSize - offset);
...
function drawSnake(ctx, snakeObj, tileSize = 8, offset = 1) {
	//halo
	if (snakeObj.id === snakeId) {
		const isoLayer = 1;
		const size = snakeObj.size/meta.tileSize;
		const qSize = meta.tileSize / 4;
		const head = snakeObj.blocks[snakeObj.blocks.length - 1];
		let x = (head.x + isoLayer + (1 - size) / 2) * meta.tileSize;
		let y = (head.y - isoLayer + (1 - size) / 2) * meta.tileSize;
		const coords = rotate(0, 0, x, y, 45);
		x = coords[0] + globalOffset[0];
		y = coords[1] / 2 + globalOffset[1];
	...
	ctx.lineCap = 'round';
	sortedBlocks = snakeObj.blocks.sort((block1, block2) => {
	...
	sortedBlocks.forEach(block => {
		...
		red.src = "./sprites/red.png"
		const isoLayer = 1;
		const size = 1;
		const qSize = tileSize / 4;
		const hSize = tileSize / 2;
		let x = (block.x + isoLayer + (1 - size) / 2) * tileSize;
		let y = (block.y - isoLayer + (1 - size) / 2) * tileSize;
		const coords = rotate(0, 0, x, y, 45);
		x = coords[0] + globalOffset[0];
		y = coords[1] / 2 + globalOffset[1];

		ctx.drawImage(red, x, y, tileSize * size + qSize, tileSize * size + qSize);
...
function drawFood(ctx, food, tileSize = 8, offset = 1) {
	...
	if (food.perishable) {
		const size = 0.4;
		let x = (food.x + isoLayer + (1 - size) / 2) * tileSize;
		let y = (food.y - isoLayer + (1 - size) / 2) * tileSize;
		const coords = rotate(0, 0, x, y, 45);
		const qSize = tileSize / 4;
		const hSize = tileSize / 2;
		x = coords[0] + globalOffset[0];
		y = coords[1] / 2 + globalOffset[1];
		ctx.drawImage(sprites.food, x, y, tileSize * size + qSize, tileSize * size + qSize);
	} else {
		const size = 1;
		const qSize = tileSize / 4;
		const hSize = tileSize / 2;
		...
		y = coords[1] / 2 + globalOffset[1];
		if (food.weight >= 4) {
			ctx.drawImage(sprites.violet, x, y, tileSize * size + qSize, tileSize * size + qSize);
		} else if (food.weight >= 3) {
			ctx.drawImage(sprites.blue, x, y, tileSize * size + qSize, tileSize * size + qSize);
		} else if (food.weight >= 2) {
			ctx.drawImage(sprites.green, x, y, tileSize * size + qSize, tileSize * size + qSize);
		} else {
			ctx.drawImage(sprites.red, x, y, tileSize * size + qSize, tileSize * size + qSize);
		}
	}
	// ctx.fillRect(
	//   food.x * tileSize,

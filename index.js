...
let tiles = NUM_TILES * scaleFacter;
		...
		switch (data) {
			...
			case 'info':
				...
				for (let i = 0; i < snakes.length; i++) {
					console.log(
						`id: [${snakes[i].id}], name: [${snakes[i].name}], msg: [${snakes[i].message
						}], ded: [${snakes[i].dead}], type:[${comps.filter(cmp => cmp.id === snakes[i].id).length > 0
							? 'C'
							: 'P'
						}]`
					);
...
setInterval(() => {
	...
	snakes.forEach(snake => {
		...
		if (!snake.dead) {
			...
			snake.blocks.push({ x: newX, y: newY, size: tileSize, type: "snakeBlock" });
			snake.blocks.splice(0, snake.blocks.length - snake.len);
			// rescaleSnakeBlocks(snake)
